...
double SED::compute_mab(float zr, std::vector <long double> &waves,std::vector <long double> &sed, std::vector <long double> &fwaves,std::vector <long double> &fresp){
  ...
  //cout << "IN mab after ionterp" << endl;
  //for (int l=0; l<sed_interp.size(); l++){
  //  cout << wave_interp[l] << " " << sed_interp[l] << endl;
  //}
  //exit(1);
...
//model C Nelson+19
  ...
  for (auto k=0;k<ex_curve.size();k++){  
    if(ex_ls[k]<2000.){
      ...
      modC.push_back(1./(ex_curve[k]*tauT)*(1.-exp(-ex_curve[k]*tauT))); 	  
    }
