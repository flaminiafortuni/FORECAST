...
void readTNGParticle::Initialize(std::string workdir, int particleID){
  ...
  snapNameTemplate = workdir+"/output/snapdir_0"+std::to_string(particleID)+"/snap_0"+std::to_string(particleID);
  fofNameTemplate = workdir+"/output/groups_0"+std::to_string(particleID)+"/fof_subhalo_tab_0"+std::to_string(particleID);

  offsetFileName = workdir+"/output/offsets/offsets_0"+std::to_string(particleID)+".hdf5";
...
void readTNGParticle::readGASElements(){
  ...
  elements_gas = Eigen::MatrixXd::Zero(naxes[1], naxes[0]); //coordinates_gas
