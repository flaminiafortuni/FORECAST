...
// map stellar particle IDs on Subhalo IDs for the TNG simulation
...
// extract from bc03/cb16 ised file
...
std::string FileStructIterator::getNext() {
    ...
    case STATE_START:
        m_state = STATE_LOOP1;
        if (m_filename.find("cb2016") != std::string::npos || m_filename.find("chab") != std::string::npos) {
	  //(m_filename.find("cb2016") != std::string::npos) {
            m_badLineNumber = 5;
        } else {
            m_badLineNumber = 6;
        }
	...
	for (std::string token; iss >> token; ){
	  if (counter==0){
		...
		for (int k=0; k<tmp_table.size(); k++)
	      ...
	      else if (tmp_table.size() == waves.size()) {//(tmp_table.size()>250){ //new
...
void readParameters(float* fov,float* res,
  ...
  if(inputf.is_open()){
    ...
    inputf >> *fov;
    inputf >> butstr; // image resolution in arcsec
    inputf >> *res;
...
bool allNegOne(const std::vector<float>& vec) {
    return std::all_of(vec.begin(), vec.end(), [](float val) { return val == -1.0; });
}
...
valarray<float> gridist_nok(vector<float> x, vector<float> y, vector<float> q,  unsigned long int nn){
  ...
  for (int i=0; i<n0; i++){
    ...
    if(grx>=0 && grx<nn && gry>=0 && gry<nn){ 
	grxy[grx+nn*gry] += quantity;
    }
  } 
  return grxy;
}
...
//create a grid with particle counts in single pixel
valarray<float> ncounts(vector<float> x, vector<float> y,  int nn){
  std::valarray<float> counter( nn*nn );
  int n0 = x.size();
